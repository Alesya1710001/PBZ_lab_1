...
INSERT INTO professor (professors_number, surname, post, department, speciality, phone_number) 
...
INSERT INTO student_group (group_number, title, number_of_students, speciality, surname_of_headman) 
...
CREATE TABLE subject(
	subject_number char(3),
    title TEXT,
    number_of_hours INTEGER, 
	speciality char(30),
    semester INTEGER
);
INSERT INTO subject (subject_number, title, number_of_hours, speciality, semester) 
VALUES ('12П', 'Мини ЭВМ', 36, 'ЭВМ', 1 ),('14П', 'ПЭВМ', 72, 'ЭВМ', 2 ), ('17П', 'СУБД ПК', 48, 'АСОИ', 4 ),
('18П', 'ВКСС', 52, 'АСОИ', 6 ),('34П', 'Физика', 30, 'СД', 6 ),('22П', 'Аудит', 24, 'Бухучета', 3 );

CREATE TABLE studying_proccess(
	group_id char(3) references student_group(group_number) on delete cascade,
    subject_id char(3) references subject(subject_number) on delete cascade,
    professors_id char(4) references professor(professors_number) on delete cascade,
    number_of_class INTEGER,
    primary key (group_id, subject_id, professors_id, number_of_class)
);

INSERT INTO studying_proccess (group_id, subject_id, professors_id, number_of_class) 
